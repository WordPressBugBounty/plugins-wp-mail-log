...
class WP_MAIL_LOG {
	...
	*/
	public function wpml_settings() {
		$setting_pages_keys = [ 'general' ];

		if ( isset( $_GET['tab'] ) ) {
			$tab = sanitize_key( wp_unslash( $_GET['tab'] ) );
			if ( in_array( $tab, $setting_pages_keys, true ) ) {
				...
				<div class="wml-settings-content-wrapper">
					<nav class="wml-nav-tab-wrapper">
						...
						foreach ( $setting_pages as $key => $label ) {
							?>
							<a class="wml-nav-tab <?php echo ( ( '' === $this->current_tab && 'general' === $key ) || $key === $this->current_tab ) ? 'wml-tab-active' : ''; ?>" href="admin.php?page=wpv-wpml-settings&tab=<?php echo esc_attr( $key ); ?>"><?php echo esc_html( $label ); ?></a>
							<?php
						}
						?>
					</nav>
	...
	*/
	...
	public function wml_admin_enqueue_scripts() {
		...
		wp_localize_script(
			...
			[
				...
				'temp_date'    => wp_date( get_option( 'date_format' ) ),
				'settings'     => $settings,
	...
	public function wml_delete_entry() {
		...
		$query = $wpdb->prepare( "DELETE FROM %i WHERE DATE_FORMAT(captured_gmt,GET_FORMAT(DATE,'JIS')) <= %s", $table_name, $date );
